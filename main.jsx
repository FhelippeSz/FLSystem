    ...
    <div style={{ display: 'flex', gap: 20, height: '100%' }}>
          ...
          {produtos.map((p) => {
            ...
            return (
              <button key={p.id} className="prod-card" onClick={() => onAdd(p)} disabled={p.estoque <= 0} style={{
                ...
                <div style={{ fontSize: 10, color: C.muted, fontFamily: "'JetBrains Mono',monospace", marginBottom: 8, display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div style={{ width: 330, flexShrink: 0, background: C.panel, border: `1px solid ${C.borderLight}`, borderRadius: 12, display: 'flex', flexDirection: 'column', height: 'fit-content', position: 'sticky', top: 0, boxShadow: '0 1px 2px rgba(20,22,26,0.03)' }}>
        ...
        <div className="scroll-thin" style={{ maxHeight: 380, overflow: 'auto', padding: '6px 10px' }}>
          ...
          {carrinho.map((i) => (
            <div key={i.id} className="table-row" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 6px', borderBottom: `1px solid ${C.borderLight}`, borderRadius: 6 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12.5, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{i.nome}</div>
                <div style={{ fontSize: 11.5, color: C.muted, fontFamily: "'JetBrains Mono',monospace" }}>{fmt(i.preco)} · {fmt(i.preco * i.qtd)}</div>
              </div>
    ...
    <div style={{ maxWidth: 920 }}>
      ...
      <form onSubmit={submitNovo}>
        <Card style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.8fr 1fr 0.9fr 0.8fr 1.3fr auto', gap: 10, alignItems: 'end' }}>
            <div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <input value={novo.codigoBarras} onChange={(e) => setNovo({ ...novo, codigoBarras: e.target.value })} placeholder="Gerar automático" style={cellInputMono} />
                <button type="button" onClick={() => setNovo({ ...novo, codigoBarras: gerarCodigoBarras() })} style={{ ...iconBtn, width: 28, height: 28, flexShrink: 0 }} title="Gerar código">
                  <Barcode size={13} color={C.muted} />
                </button>
              </div>
            </div>
