    ...
    return (
      <div>
        ...
        >
          <Icon size={16} strokeWidth={2} />
          {label}
          {mostrarAberto ? <ChevronDown size={14} style={{ marginLeft: 'auto' }} /> : <ChevronRight size={14} style={{ marginLeft: 'auto' }} />}
        </button>
        {mostrarAberto && <div style={{ display: 'flex', flexDirection: 'column', gap: 1, marginTop: 1 }}>{itens.map(renderSubNavBtn)}</div>}
      </div>
    );
  }
